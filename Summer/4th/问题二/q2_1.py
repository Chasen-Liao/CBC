...
def solve_supplier_selection(
    n_suppliers: int = 50, 
    capacity_requirement: float = 28200 * (1 + 0.0139), # 这把平均损耗加上了 
    p_C: float = 1.0
) -> Tuple[Dict[int, int], Dict[int, float], int, float, float]:  # 增加返回总产能
    """
    ...
    supply_amounts: 供应商供应量字典 {供应商ID: 供应量}
    min_count: 最小供应商数量
    min_cost: 最小总成本
    total_capacity: 总产能
    """
    ...
    # 计算有效供应量 eff_supply_i = 平均供货量 × (1 + 超额供货率) * (1 - 短缺供货率)
    ...
    # 步骤5: 求解模型
    # ====================================================
    solver = pulp.PULP_CBC_CMD(timeLimit=100, msg=True)
    model.solve(solver)
    ...
    for i in df_suppliers.index:
        ...
        if selected == 1:
            ...
            if material_type == 'A':
                total_capacity += supply_amount / 0.6
            elif material_type == 'B':
                total_capacity += supply_amount / 0.66
            elif material_type == 'C':
                total_capacity += supply_amount / 0.72
    ...
    for supplier_id in list(supply_amounts.keys())[:n_suppliers]:
        ...
        material = df_suppliers.loc[df_suppliers['supplier_id'] == supplier_id, 'material_type'].values[0]
        
        # 计算产能贡献（如果被选择）
        capacity_contribution = 0.0
        if selected_suppliers[supplier_id] == 1:
            if material == 'A':
                capacity_contribution = supply / 0.6
            elif material == 'B':
                capacity_contribution = supply / 0.66
            elif material == 'C':
                capacity_contribution = supply / 0.72
        
        print(f"供应商 {supplier_id} ({material}类): {status}, 供应量 = {supply:.2f} 立方米, 产能贡献 = {capacity_contribution:.2f} 立方米")
    
    return selected_suppliers, supply_amounts, min_count, min_cost, total_capacity  # 增加返回总产能

if __name__ == "__main__":
    
    selected, supply_amounts, min_count, min_cost, total_capacity = solve_supplier_selection(n_suppliers=50)